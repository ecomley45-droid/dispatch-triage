...
export function computeOverview({ invoices = [], workOrders = [], lines = [], timeEntries = [], customers = [], plans = [], now = Date.now() }) {
  ...
  const invoicedThisMonth = round2(issued.filter((i) => i.issue_date && monthKey(new Date(i.issue_date)) === thisMonth).reduce((s, i) => s + Number(i.total || 0), 0));
  ...
  for (const i of issued) {
    ...
    const b = bucketByKey[monthKey(new Date(i.issue_date))];
