...
export const CAPABILITIES = {
  ...
  // Bulk CSV/XLSX data import (customers/sites/assets/items/members) — a
  // high-blast-radius write (can touch hundreds of records at once, and can
  // invite teammates), so kept to the same tier as members:write/roles-adjacent
  // powers rather than opened to accountant/dispatcher.
  'data:import': ['org_admin', 'manager_admin'],
  // Self-service workspace branding (logo, colors, font, display name) —
  // org-admin-only, same tier as roles:write/regions:write: it changes what
  // every user in the workspace sees, not day-to-day operations.
  'branding:write': ['org_admin'],
  // Enterprise-tier only (see ENTERPRISE_FLAGS below) — configuring the
  // workspace's SAML/SSO connection. org-admin-only like integrations:write,
  // and likewise excluded from PAGES[].caps so it's never offered in the
  // custom Role Editor: this is a hardcoded org_admin power, not something a
  // workspace can delegate.
  'sso:write': ['org_admin'],
  // Public API keys + outbound webhooks — same tier as integrations:write
  // (org-admin-only, excluded from the custom Role Editor): a leaked/misused
  // key or a malicious webhook target has workspace-wide blast radius.
  'api_keys:write': ['org_admin'],
};
...
export const CAP_LABEL = {
  ...
  'data:import': 'Bulk-import data (CSV/XLSX)',
  'branding:write': 'Customize workspace branding (logo, colors, font)',
  'sso:write': 'Configure single sign-on (SAML/SSO)',
  'api_keys:write': 'Manage API keys & webhooks',
};
...
export const PAGES = [
  ...
  { key: 'imports', label: 'Import Data', path: '/imports', collections: [], caps: ['data:import'], navRoles: ['org_admin', 'manager_admin'] },
  { key: 'settings', label: 'Settings', path: '/settings', collections: ['service_offers'], caps: ['service:write', 'members:write', 'roles:write', 'branding:write'], alwaysVisible: true },
];
...
// Capabilities gated by workspace PLAN TIER (orgs.plan, set by the platform
// Super Admin), not by an org-admin-togglable feature flag. A 'starter'
// workspace never sees these, regardless of role/permission configuration —
// this is a sales-tier gate, not a per-workspace preference. Same
// strip-everywhere-at-once enforcement shape as FEATURE_FLAGS (see
// enterpriseCaps below + attachPerms in lib/auth.js).
export const ENTERPRISE_FLAGS = [
  { key: 'sso', label: 'Single sign-on (SAML/SSO)', caps: ['sso:write'] },
];
export const isEnterprisePlan = (plan) => plan === 'enterprise';
// Caps to strip because the workspace's plan doesn't include them — empty
// once the workspace IS on an enterprise plan.
export const enterpriseCaps = (plan) =>
  isEnterprisePlan(plan) ? [] : ENTERPRISE_FLAGS.flatMap((f) => f.caps || []);
