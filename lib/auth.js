...
// The single owner: the primary ADMIN_EMAILS account (or the dev-bypass admin
// locally). Used to gate the owner-only executive dashboard.
export const isOwner = (viewer) => {
  if (!viewer) return false;
  if (viewer._dev) return true;
  const primary = adminEmails()[0];
  return !!primary && viewer.email?.toLowerCase() === primary;
};
export const requireOwner = (req, res, next) => {
  if (!req.viewer) return res.status(401).json({ error: 'Authentication required' });
  if (!isOwner(req.viewer)) return res.status(403).json({ error: 'Owner only' });
  next();
};
