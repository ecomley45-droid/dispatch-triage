...
export const CAPABILITIES = {
  ...
  // Photos + timestamped notes on jobs/projects — the dispatch team's comms.
  'attachments:write': ['manager_admin', 'accountant_admin', 'dispatcher'],
  // Optional AI drafting assistant — available to every member (advisory only).
  'ai:use': ['manager_admin', 'accountant_admin', 'dispatcher'],
  // CRM: customer accounts + billing terms are an admin/accounting concern.
  'customers:write': ['manager_admin', 'accountant_admin'],
  // Sites + assets are operational — the dispatch team maintains them.
  'sites:write': ['manager_admin', 'accountant_admin', 'dispatcher'],
  'assets:write': ['manager_admin', 'accountant_admin', 'dispatcher'],
  // Work orders are the dispatch team's core object.
  'work_orders:write': ['manager_admin', 'dispatcher'],
  // Line items carry pricing — dispatch adds labor, accounting sets rates.
  'wo_lines:write': ['manager_admin', 'accountant_admin', 'dispatcher'],
  // Invoicing / A/R is a billing concern — manager + accountant only.
  'invoices:write': ['manager_admin', 'accountant_admin'],
  'invoice_lines:write': ['manager_admin', 'accountant_admin'],
  // Final sign-off that a job is truly done — managers only.
  'work_orders:approve': ['manager_admin'],
  // Reviewing/approving timesheet correction requests — managers only.
  'timesheets:review': ['manager_admin'],
  // Reading the audit log — managers only.
  'audit:read': ['manager_admin'],
  // Recurring/preventive maintenance plans.
  'maintenance:write': ['manager_admin', 'accountant_admin', 'dispatcher'],
  // Financial reports & exports — owner + accounting.
...
export async function resolveViewer(req, _res, next) {
  try {
    ...
    if (!clerkConfigured() && !isProd()) {
      ...
      // Honor the requested workspace if the dev user is a member of it; else
      // fall back to their default membership, then the seeded workspace.
      req.org = (slug && await store.orgForUser('dev@localhost', slug))
        || (await store.orgForUser('dev@localhost'))
        || { id: 'family-dental', slug: 'family-dental', name: 'Family Dental Health', role: 'manager_admin', feature_flags: {} };
      return next();
    }
    ...
    req.viewer = viewer;
    // Prefer the URL-selected workspace (membership-checked); fall back to the
    // caller's default membership so /api/me and single-workspace users work.
    req.org = viewer
      ? (slug && await cachedOrgForUser(viewer.email, slug)) || await cachedOrgForUser(viewer.email)
      : null;
