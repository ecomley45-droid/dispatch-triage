...
import { CAPABILITIES, PRESET_ROLES, PAGE_KEYS, presetPerms, sanitizePerms, isRestrictedRole, disabledFeaturePages, disabledFeatureCaps, featureActive } from './permissions.js';
...
async function viewAsOrg(req, viewer) {
  ...
  if (!org) return null;
  return { id: org.id, slug: org.id, name: org.name, role: role || 'org_admin', feature_flags: org.feature_flags || {}, branding: org.branding || {}, viewingAs: true };
}
...
export async function resolveViewer(req, _res, next) {
  try {
    ...
    // ecosystem instance (Phase C pilot, lib/dualAuth.js) — never the
    // reverse, an own-instance session above always wins when present.
    const viewer = userId ? await loadClerkUser(userId) : await loadSharedViewer(req).then((shared) => shared && {
      ...shared,
      role: adminEmails().includes(shared.email) ? 'org_admin' : 'dispatcher',
    });
...
async function attachPerms(req) {
  ...
  const offCaps = new Set(disabledFeatureCaps(flags));
