...
const sbApi = {
  ...
  async orgForUser(email) {
    ...
    // `branding` is a newer column; on a deploy that lands ahead of its
    // migration, selecting it errors. Fall back to the pre-branding shape so
    // the client site still resolves the workspace (branding just defaults).
    let { data, error } = await db().from('org_members').select('org_id, role, orgs(id, name, feature_flags, branding)')
      .eq('user_email', user).limit(1).maybeSingle();
    if (error && isMissingSchema(error)) {
      ({ data } = await db().from('org_members').select('org_id, role, orgs(id, name, feature_flags)')
        .eq('user_email', user).limit(1).maybeSingle());
    }
