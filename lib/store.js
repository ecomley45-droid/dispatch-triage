...
const memApi = {
  ...
  // Upsert name/default_region_id (and optionally permissions) for a role key —
  // used to override a built-in preset's name or default region.
  async setRoleMeta(orgId, key, patch) {
    seedDemo();
    const id = `${orgId}:${key}`;
    const cur = mem.roles.get(id) || { org_id: orgId, key, name: null, permissions: {}, default_region_id: null, created_at: now() };
