...
export async function seedDemoInto(orgId, byEmail = 'demo@dispatch') {
  ...
  const owner = byEmail;
  const created = { customers: 0, sites: 0, assets: 0, work_orders: 0, work_order_lines: 0, invoices: 0, invoice_lines: 0, items: 0, service_offers: 0, projects: 0, jobs: 0, time_entries: 0, item_usage: 0, shifts: 0, timesheet_requests: 0, punch_items: 0, maintenance_plans: 0 };
  ...
  const riverside = await ins('customers', { name: 'Riverside Dental Partners', billing_email: 'ap@riverside-dental.example', phone: '(555) 210-4488', billing_address: '900 Riverside Blvd, Suite 200', payment_terms: 'net_30', po_required: true, status: 'active', notes: 'Multi-location group; PO required on all work.', portal_token: randomUUID(), created_by: owner });
  ...
  const cedar = await ins('customers', { name: 'Cedar Family Dentistry', billing_email: 'billing@cedarfamily.example', phone: '(555) 774-1902', billing_address: '410 Cedar St', payment_terms: 'due_on_receipt', po_required: false, status: 'active', notes: 'Single location; pays on receipt.', portal_token: randomUUID(), created_by: owner });
  ...
  // Preventive-maintenance plans (one due now to demo "Generate due", one upcoming).
  await ins('maintenance_plans', { customer_id: cedar.id, site_id: cMain.id, asset_id: sterilizer.id, title: 'Quarterly autoclave PM', description: 'Routine sterilizer preventive maintenance.', priority: 'medium', frequency: 'quarterly', assignee_email: tech, next_due: iso(-1 * DAY).slice(0, 10), active: true, created_by: owner });
  await ins('maintenance_plans', { customer_id: riverside.id, site_id: rNorth.id, asset_id: compressor.id, title: 'Compressor filter service', description: 'Replace intake filter, check pressure.', priority: 'low', frequency: 'quarterly', assignee_email: null, next_due: iso(20 * DAY).slice(0, 10), active: true, created_by: owner });
